...
anno = {"info": {"description": "Passport Dataset", "url": "http://cocodataset.org", "version": "1.0", "year": 2017,
        ...
        "images": [],
        "annotations": [],
        "categories": [{"supercategory": "id_card", "id": 1, "name": "id_card"}, ]
        }
...
def image_on_background(img):
    ...
    background = cv2.imread(random.choice(glob.glob("background_1/*.jpg")))
...
def create_data(folder_name,count):
    ...
    for i in range(count):
        img=cv2.imread(random.choice(glob.glob('real_id/*.png')))
...
create_data('coco_json/train',640)
create_data('coco_json/test',80)
create_data('coco_json/valid',80)